...
class PositionalEncoding(nn.Module):
    def __init__(self, pe_embed, noise = 'none'):
    ...
    def forward(self, pos):
        if self.pe_embed == 'none':
            ...
            pe_list = []
            for i in range(self.levels): # levels * 2  = d = embed_length
                temp_value = pos * self.lbase **(i) * math.pi
                pe_list += [torch.sin(temp_value), torch.cos(temp_value)]
            if self.noise == 'none':
                return torch.stack(pe_list, 1)
            elif self.noise == 'gaussian':  ##标准正态分布N（0,1）
                noise_mask = torch.randn((pos.size(0), self.embed_length))
                ape = torch.stack(pe_list, 1)
                return ape * torch.tensor(noise_mask).to(ape.device)
            elif self.noise == 'uniform':  # 均匀分布 (0, 1)
                noise_mask = torch.rand((pos.size(0), self.embed_length))
                ape = torch.stack(pe_list, 1)
                return ape * torch.tensor(noise_mask).to(ape.device)
            elif self.noise[:8] == 'designed':
                # 截断正态分布
                mean = 0.0       # 均值
                _, std = [x for x in self.noise.split('_')]
                std = float(std)   # 标准差
                low = -1.0       # 截断下限
                high = 1.0       # 截断上限
                a, b = (low - mean) / std, (high - mean) / std
                truncated_dist = truncnorm(a, b, mean, std)
                noise_mask = truncated_dist.rvs(size=(pos.size(0), self.embed_length))
                ape = torch.stack(pe_list, 1)
                return ape * torch.tensor(noise_mask).to(ape.device).to(torch.float32)
            elif self.noise == 'bernoulli':
                # 伯努利分布 (p=0.5)
                noise_mask = torch.bernoulli(torch.full((pos.size(0), self.embed_length), 0.5))
                ape = torch.stack(pe_list, 1)
                return ape * torch.tensor(noise_mask).to(ape.device)
...
class Keycontrol_PE(nn.Module):
    def __init__(self, pe_embed, key=''):
        super(Keycontrol_PE, self).__init__()
        ...
        # self.embed_layer = nn.Sequential(nn.Conv2d(1, self.levels, kernel_size=[1, 1], padding='same'),
        #                                  nn.BatchNorm2d(self.levels),
        #                                  nn.ReLU())
        #
        # self.embed_layer2 = nn.Sequential(
        #     nn.Conv2d(self.levels * 4, self.levels, kernel_size=[1, 1], padding='valid'),
        #     nn.BatchNorm2d(self.levels),
        #     nn.ReLU())
        self.FeedForward = nn.Sequential(
            nn.Linear(1, self.levels * 2, bias=True),
            nn.GELU(),
            nn.Linear(self.levels * 2, self.levels * 2, bias=True),
            nn.GELU(),
            # nn.Dropout(dropout),
            # nn.Linear(self.levels * 2, self.levels * 2)
            # nn.Dropout(dropout)
        )
        self.out = nn.Linear(self.levels * 2, self.levels * 2, bias=True)
        self.Pe = PositionalEncoding(self.pe_embed)
    def forward(self, pos, train_pe=False):
        x = self.FeedForward(pos.view(pos.size(0), 1))
        x = self.out(x)
        ...
        if not train_pe:
            ...
            return x
        # np.savetxt('expe_data/KCPE_tensor{}.txt'.format(pos.cpu().numpy().astype(np.float16)), x.cpu().numpy().astype(np.float16))
        # np.savetxt('expe_data/FKCPE_tensor{}.txt'.format(pos.detach().cpu().numpy().astype(np.float16)),
        #            x.detach().cpu().numpy().astype(np.float16))
...
if __name__ == '__main__':
    PE = PositionalEncoding(pe_embed='1.25_80')
    pe = []
    for i in range(int(200)):
        pe = PE(torch.tensor([i/10000]))
        if i == 0:
            a = pe
        else:
            a = torch.cat((a, pe), dim=0)

    np.savetxt('expe_data/tensor.txt', a.numpy().astype(np.float16))
